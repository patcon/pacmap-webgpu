...
Point radius is deliberately **screen-constant** under zoom: the quad offset is added after projection and multiplied by `clip.w`, cancelling the perspective divide, so zooming resolves a dense cluster instead of magnifying it. The reference project this borrows from does the opposite; switching is one line. In 3D that rule is arguably the wrong one — the analogue of "zoom resolves rather than magnifies" there is per-point distance scaling, which is the strongest size cue the eye has and is currently thrown away. See `TODO.md`.
