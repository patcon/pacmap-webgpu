...
- **The depth is clamped, not the size** — `max(clip.w, REF_DIST / MAX_GROW)`. One expression caps growth at 4x *and* disposes of `w <= 0` for points behind the camera, which Orbit's `minDistance` makes reachable and which would invert the quad. In-cloud variation at the default framing is only ~0.73x–1.6x, so the cap bites only once the camera is among the points.
