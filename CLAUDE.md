...
- **Orbit's defaults are wrong here in two specific ways**, both fixed at construction. Its mouse map is left-rotate / right-pan, and `onMouseDown` tests ORBIT first and returns early when rotation is disabled — so pointing `PAN` at button 0 is not enough, `ORBIT` has to move off it too. And pan sensitivity is set by `panSpeed` and `inertia` *together*: `update()` adds the whole of `panDelta` to the target every frame and only decays it by `inertia` afterwards, never clearing it, so one drag delta lands `d/(1 - inertia)` = 6.7x over at the default 0.85. Orbit's `panSpeed: 0.1` is tuned against that coasting, not against its pixel-to-world conversion — which is itself exactly 1:1. `panSpeed: 1, inertia: 0` is what makes a drag track the cursor exactly, which is the intended feel here; measured 1.000x against 6.667x for `panSpeed: 1` alone. Pan is never eased (the target moves directly), so `ease` is left at its default and only smooths the zoom.
