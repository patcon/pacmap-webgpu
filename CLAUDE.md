...
The `rendering` folder's checkbox (on by default) makes playback smooth rather than stepped, which matters most at exactly the sizes where `stride > 1` — above N≈37k in 2D and N≈25k in 3D, where consecutive banked frames are several optimizer iterations apart. `drawFrame` takes the **fractional** playhead rather than a rounded slot, binds slots `a` and `b = min(a+1, banked-1)` as two vertex buffers, and the vertex stage does `mix(p, pB, V.lerpT)`. This is the sibling project's mechanism (`marimo-pacmap-animation/app/app.js`), minus its dequantization: a plain lerp, no easing and no spline. Off pins `lerpT` to 0, where `mix` returns `p` exactly, so unticking it gives back the stepped playback bit for bit.

Four things carry it:

- **A `playback fps` slider had to come with it.** Playback used to advance at a fixed `PLAYBACK_FPS = 60`, so on a 60Hz panel the cursor lands on a keyframe every rAF tick and there is no in-between to draw — the checkbox would have appeared to do nothing. The default is now 30 (two drawn frames per keyframe pair at 60Hz), and the slider tops out at 60, above which it only skips keyframes.
- **The framing box is mixed on the same `t`.** Under `auto zoom` each slot is framed by its own banked bound, and gliding points under a snapping box pulse once per keyframe. So there are **two** `Bounds` uniforms — `@binding(0)` and `@binding(3)` — and the shader mixes `lo`/`hi` before computing `ctr`/`span`. Two 32-byte bindings rather than one 64-byte struct, because `BOUNDS_BYTES` means both the history slot stride *and* a uniform's size, and the staging buffer and the `sessionStorage` schema are written against that one constant. With auto zoom off both bindings get the same box and the mix is a no-op.
- **The live path binds one keyframe to both slots**, and writes one box into both bounds uniforms. A running frame has no next keyframe to interpolate toward, so the mix has to come out at what it was given whatever `lerpT` holds; `encodeRender`'s `offsetB` defaults to `offset` for exactly that.
- **The scrubber is `step="any"`, not `step="1"`.** The fraction is the whole mechanism, so a quantized slider pins every dragged position to a keyframe and hides the interpolation from the one interaction where it is easiest to see. The cost is that the browser's native arrow-key stepping on a focused range becomes 1% of the range rather than one keyframe, so the window `keydown` handler stops exempting the scrubber and does whole keyframes itself.
- **`lerpT` is the last of the view uniform's 24 floats** and gets its own 4-byte `writeBuffer` per drawn frame (`LERP_T_OFFSET`), because it changes every frame while the rest of that struct changes only when the camera or a pane control moves. `resize()` writes it too, from the same variable, so a mid-playback resize cannot stomp it back to 0. The struct is now exactly 96 bytes rather than 92 padded to 96 — `lerpT` took the padding, as `occlude` took the slot before it.

The interpolation is invisible to the headless checks — nothing renders a pixel — but `check:shaders` does cover the two mistakes that would blank the canvas: the fourth vertex buffer is declared at both dimensionalities, and the occlusion-draw case sets it and binds the second box, so one bind group and one vertex layout still have to serve both render pipelines.
