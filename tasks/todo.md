...
- [x] **3. `pair graph` folder: three sliders, greying, LocalMAP note** — M — deps: 2
      ...
      - [x] `show edges` checkbox; `near %`, `mid-near %`, `further %` (0–100, step 0.5,
            defaults 100 / 100 / 5), generated by iterating `EDGE_KINDS`
      ...
      - [x] The labels carry what a colour cannot: `near % (attract)`,
            `mid-near % (w=0 in phase 3)`, `further % (repel)`. The mid-near one is the
            place where "an edge means this pair exists" would otherwise overclaim
