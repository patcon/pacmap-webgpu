...
- [x] **2. Edge shader, both pipelines, index buffer, draw** — L — deps: 1
      ...
      - [x] `view.edges = false` plus a `show edges` checkbox in `rendering` (the plan's
            ...
            `view.edgePct` hard-coded until Task 3
...
- [x] **3. `pair graph` folder: three sliders, greying, LocalMAP note** — M — deps: 2
      `src/main.ts`
      - [x] New `pair graph` folder below `rendering` — its own folder rather than more
            `rendering` rows, because it is about the algorithm's structure where that
            folder is about how the cloud is drawn. Live all the same
      - [x] `show edges` checkbox; `% neighbors (pos)`, `% mid-near (pos)`, `% far (neg)`
            (0–100, step 0.5), generated by iterating `EDGE_KINDS`. Defaults settled at
            **10 / 1 / 1** after seeing it: what is wanted is the shape of each relation,
            not the whole of it, and mid-near and far both span the cloud
      - [x] Each slider resolves against its range total on the CPU and moves a draw
            count; the shader learns nothing new
      - [x] 0% encodes no draw rather than a zero-count draw (landed in Task 2)
      - [x] Folder disabled under both CPU engines, from the same `syncAlgorithm` that
            greys `kNN algo` — and `go()` builds no overlay there anyway, since
            `EmbeddingRun.graph` is undefined
      - [x] The labels name the **sign of the force**, which a colour does not say. The
            `w_MN = 0` caveat did not survive the rename — `(pos)` overstates mid-near for
            the last 250 iterations — so it lives in `CLAUDE.md` and `README.md` instead
      - [x] Under LocalMAP the **folder title** gains
            `— further pairs are the initial draw`, since Tweakpane has no tooltips and
            the title is the one string that can change with the dropdown
      - [x] `build`, `check:shaders` (15), `check:kernels` (68), `check:druid` (20) green;
            `check:ab` bit-identical on both variants
      - [x] No new headless checks, and none available: this is pane wiring, which
            `CLAUDE.md` already records as browser-only
...
### ⬜ Checkpoint: pane complete
- [ ] Every control exercised by hand under both engines

## Phase 4 — Documentation

- [x] **4. `CLAUDE.md` + `README.md`** — S — deps: 3
      - [x] New `The pair graph overlay` section, six points: indexed line-list over the
            existing position buffer (and why not a storage buffer — 256-byte
            dynamic-offset alignment vs. `frameBytes`, and `posHistory` already at the
            128MB binding limit); why the kind rides in a dynamic-offset uniform
            (`vertex_index` is the *fetched index* on an indexed draw); the shared
            struct/world **text** and why it is spliced rather than factored into a WGSL
            function; why the depth pipeline is mandatory; why the ranges are shuffled at
            setup; why `buildEdgeIndices` has its own module
      - [x] Plus the two things the overlay does *not* say — nothing varies with weight,
            and `w_MN = 0` through phase 3, which the renamed slider label now overstates
      - [x] Plus the counts, since they get asked: 10/5/20 at the defaults, but every
            pair acts on both endpoints so a point's degree is about twice that — which
            is why all three default low
      - [x] LocalMAP section: the overlay shows the initial further pairs, not the
            resampled ones, tied to the no-readback design rather than presented as an
            oversight, and what reflecting it would take
      - [x] Coverage gaps: the split between what *is* checked (pipelines, and the index
            buffer as data) and what stays browser-only (whether an edge is drawn on its
            endpoints, a pixel question) — plus the two plausible-looking mutations that
            do **not** fail, so nobody adds them as checks
      - [x] `check:shaders` paragraph: `src/edges.ts` exists for the same reason
            `src/shaders.ts` does, and there are now two draw-encoding cases
      - [x] Pipeline list gains `src/edges.ts`; pane paragraph gains the `pair graph`
            folder, its lifetime and its greying
      - [x] `README.md` gains `Seeing the pairs`: the three colours, what each set does,
            that every drawn pair is a pair that acts, the degree-vs-draw-count trap, the
            inert yellow, render-time percentages, and both limitations
      - [x] `build`, `check:shaders` (15), `check:kernels` (68), `check:druid` (20) green;
            `check:ab` bit-identical on both variants

### ✅ Checkpoint: full feature
- [x] All four checks green
- [ ] **Browser (yours):** the Task 2 and Task 3 steps above
